...
par(mfrow=c(1,2))
barplot(table(kobe_streak),main="Kobe's streak lengths")
barplot(table(sim_streak),main="Simulated streak lengths")
```

The longest streak here was 11 hits in a row, the majority of shots were missed.Therefore, distribution is unimodal and right skewed.
...
```{r}
length(which(kobe_streak >= 2))
```


```{r}
length(which(sim_streak >= 2))
```


Kobe’s streak length distribution is extremley similar to the model simulated. In conclusion, the simulated shooter model proved to have a longer max streak length than Kobe Bryant.Simulation shows 6 baskets in a row, whereas of 4 for Kobe. The majority of streaks, in each simulation, are either lengths of zero or one. This dataset shows that Kobe Byrant never had any streaks greater than or equal to 2. Thus suggests that we can not accept the hot hands model based on current evidence.
