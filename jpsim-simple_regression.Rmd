...
```{r}
my_own_model <- lm(runs ~ hits, data = mlb11)
...
```{r}
summary(my_own_model)
```
    
While the $R^2$ alue for the at-bats predictive model is $37.29%$ $R^2$ value for my model is $64.19%$, a significant increase. 

The predictor is better, it accounts for more variation.
...
After examining the summary statistics and correlation coefficients of all three new predictors new_onbase, new_slug and new_obs, the relationship between runs and new_obs variable has the highest $R2$ and the coefficient correlation values appear to be the best and most effective predictor of the runs.
...
Based on above plots, the variability of points around the least squares line remains marginnally constant. Therefore, condition constant variability has been met.
